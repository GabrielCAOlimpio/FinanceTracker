...
import {
  ArrowLeft, Calendar as CalendarIcon, TrendingUp, TrendingDown,
  Utensils, Car, Film, ShoppingBag, Heart, GraduationCap,
  Briefcase, Wallet, Gift, MoreHorizontal, Home,
} from "lucide-react"
...
function AddTransactionForm() {
  ...
  const validateForm = () => {
    ...
    else if (parseFloat(formData.amount) <= 0) newErrors.amount = "Valor deve ser maior que zero"
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <FieldGroup>
            ...
            <Field data-invalid={!!errors.category}>
              ...
              {errors.category && <FieldError id="category-error">{errors.category}</FieldError>}
            </Field>
          ...
          <div className="flex gap-3 pt-2">
            ...
            <Button
              type="submit"
              className={cn("flex-1", formData.type === "income" && "bg-success hover:bg-success/90")}
              disabled={isLoading}
            >
              {isLoading ? <><Spinner className="mr-2" />Salvando...</> : "Salvar"}
            </Button>
          </div>
        </form>
      </CardContent>
